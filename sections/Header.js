...
const Header = () => {
  ...
  const renderThemeChanger = () => {
    ...
    if (currentTheme === 'dark') {
      return (
        <Button className="bg-gray-200 dark:bg-gray-900" onClick={() => setTheme('light')}>
  ...
  return (
    <header className='h-16 flex items-center justify-between'>
      <ul className='flex gap-4'>
        {navigations.map(nav => (
          <Link key={nav.label} href={nav.path}><a className='font-semibold text-gray-400 hover:text-gray-500'>{nav.label}</a></Link>
        ))}
      </ul>
      {renderThemeChanger()}
    </header>
